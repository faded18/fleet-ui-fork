...
const Dashboard = () => {
  ...
  const { sendMessage, loadChatHistory } = useChat();
  const conversations = useSelector((state: any) => state.chat.conversations);
  const messages = useSelector((state: any) => state.chat.messages);
  const activeConversation = useSelector((state: any) => state.chat.activeConversation);
  ...
  return (
    <div className="flex-1 flex flex-col md:flex-row h-[calc(100vh-66px)] bg-gray-50">
      ...
      {(!isMobile || activeConversation) && (
        ...
        showBackButton={isMobile && !!activeConversation}
        onBack={() => loadChatHistory("")}
      />
      )}
    </div>
  );
